...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (rows.length === 0) return { statusCode: 401, headers, body: JSON.stringify({ error: '账号或密码错误' }) };
    const orderRows = await sql`SELECT COUNT(*) as cnt FROM orders WHERE username = ${username}`;
    return { statusCode: 200, headers, body: JSON.stringify({ success: true, username: rows[0].username, balance: parseFloat(rows[0].balance), orderCount: parseInt(orderRows[0].cnt) }) };
