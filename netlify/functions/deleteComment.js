...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!commentId || !username) return {statusCode:400,headers:h,body:JSON.stringify({error:'参数错误'})};
    const sql = neon();
    const rows = await sql`SELECT author FROM comments WHERE id = ${commentId}`;
    if (!rows.length) return {statusCode:404,headers:h,body:JSON.stringify({error:'评论不存在'})};
